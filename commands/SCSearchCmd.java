    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        public void playlistLoaded(AudioPlaylist playlist) {
            builder.setColor(event.getSelfMember().getColor())
                    .setText(event.getClient().getSuccess()+" xD Search results for `"+event.getArgs()+"`:")
        ...
        public void noMatches() {
            m.editMessage(event.getClient().getWarning()+" Ok bye xD No results found for `"+event.getArgs()+"`.").queue();
        }

        @Override
        public void loadFailed(FriendlyException throwable) {
            if(throwable.severity==Severity.COMMON)
                m.editMessage(event.getClient().getError()+"xD Error loading: "+throwable.getMessage()).queue();
            else
                m.editMessage(event.getClient().getError()+"lel Error loading track.").queue();
        }
    }
}
